...
@RestController
@RequestMapping("/api/exportAccountJob")
public class ExportAccountController {
    public static final Logger log = LoggerFactory.getLogger(ExportAccountController.class);

    @Autowired
    private JobLauncher jobLauncher;

    @Autowired
    @Qualifier("exportAccountJob")
    private Job exportAccountJob;

    @Value("${account.file.output}")
    String accountFileOutput;
    ...
    public ResponseEntity<BatchJobResponse> runJob() {
        ...
        try {
            JobParameters jobParameters = new JobParametersBuilder()
                    ...
                    .addString("accountFileOutput", accountFileOutput)
                    .toJobParameters();
            JobExecution jobExecution = jobLauncher.run(exportAccountJob, jobParameters);
